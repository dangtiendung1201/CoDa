...
df = pd.read_csv(base_dir / "tsv_data" / "inp_data" / f"{args.dataset}_{args.split}.tsv", sep="\t", header=0)

if '0' in df.columns:
    df = df.rename(columns={'0': 'text', '1': 'label'})

# Specify the file path
file_path = base_dir / "generation_data" / f"{args.dataset}_{args.split}_abs_prompt_{{}}" / "generated_predictions.jsonl"
solo_file_path = base_dir / "generation_data" / f"{args.dataset}_{args.split}_solo_constraint" / "generated_predictions.jsonl"
...
for k in [1, 2]:
    with open(str(file_path).format(k), 'r') as file:
        i = 0
        lines = []
        count  = 0
        for line in file:
...
df.to_csv(base_dir / "tsv_data" / "out_data" / args.dataset / f"{args.dataset}_{args.split}_ori+aug.tsv", sep="\t", index=False)
